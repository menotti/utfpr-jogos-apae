...
public class Menu extends GameObject {
    ...
    protected static final int charHeight = 15; // Altura da fonte
    protected static final int charWidth = 15; // Largura da fonte
    ...
    private int selectorWidth = 5;// = selector.length();
    private int selectorHeight = 5;// = selector.length();
    ...
    public Menu(String[] items, int x, int y, int width, MenuListener listener) {
        ...
        setWidth(width);
        setHeight(items.length * (charHeight + lineSpace) + borderSpace * 2);
        if (x == center) {
        ...
        for (int i = 0; i < items.length; i++) {
            options[i] = new Option(items[i],
                    x + borderSpace + charWidth * 2,
                    y + (charHeight + lineSpace) * i);
        }
    ...
    public void setSelectorImg(Image selectorImg) {
        ...
        selectorWidth = selectorImg.getWidth(null);
        selectorHeight = selectorImg.getHeight(null);
    }
    ...
    protected void paintBox(Graphics g) {
        ...
        g.fillRoundRect(getX(), getY(),
                getWidth(), (items.length - 1) * (charHeight + lineSpace) + borderSpace * 2,
                10, 10);

        g.setColor(colorBack); //desenha um quadrado acinzentado
        g.fillRoundRect(getX() + borderSize, getY() + borderSize,
                getWidth() - borderSize * 2,
                (items.length - 1) * (charHeight + lineSpace) + borderSpace * 2 - borderSize * 2,
                10, 10);
    }
    ...
    protected void paintItem(int x, int y, String item, Graphics g) {
        g.setFont(font);
        g.drawString(item,
                x + borderSpace + selectorWidth + charWidth * 2,
                y + (charHeight + lineSpace) + borderSpace);
    }
    ...
    protected void paintItem(int x, int y, String itens[], Graphics g) {
        ...
        for (int i = 0; i < itens.length; i++) {
            ...
            if (i != selectedIndex) {
                ...
                g.drawString(text,
                        x + borderSpace + selectorWidth + charWidth * 2,
                        y + (charHeight + lineSpace) * i + borderSpace);
            }
        }
    }
    ...
    protected void paintSelector(int x, int y, Graphics g) {
        if (selectorImg != null) {
            g.drawImage(selectorImg, x + borderSpace,
                    y + (charHeight + lineSpace) * selectedIndex  + (selectorWidth / 2), null);
        } else {
            g.setFont(font);
            g.drawString(selector, x + borderSpace,
                    y + (charHeight + lineSpace) * selectedIndex + borderSpace);
        }
    ...
    public void paint(Graphics g) {
        ...
        paintSelectedItem(x + borderSpace + selectorWidth + charWidth * 2,
                y + (charHeight + lineSpace) * selectedIndex + borderSpace, g);
